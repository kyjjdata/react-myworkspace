...
// state : 이전 상태 == 이전 state의 값 또는 객체
// action: conponent -> dispatcher로 부터 전달 받은 action 객체

// reducer는 이전 state와 action 객체를 받아서 state를 변경
const initialState = [
  {
    id: 1,
    memo: "React 공부하기",
    comments: [
      { id: 1, contect: "커멘트입니다1." },
      { id: 2, contect: "커멘트입니다2." },
    ],
  },
  {
    id: 2,
    memo: "Javascript 공부하기",
    comments: [
      { id: 3, contect: "커멘트입니다3." },
      { id: 4, contect: "커멘트입니다4." },
    ],
  },
];
...
const todo = (state = initialState, action) => {
  ...
  switch (action.type) {
    // action = {type: 'ADD_TODO', payload: {id:1, memo:'redux 공부'}}
    case "ADD_TODO":
      ...
      // let nweStudent3 = {...student};

      return [{ ...action.plaload }, ...state];

    case "REMOVE_TODO":
      // action = {type : 'REMOVE_TODO', payload:1}
      // 배열에서 요소 삭제 -> 배열크기가 변경됨 == 특정 조건에 맞지않는 요소만 리턴됨 == filter
